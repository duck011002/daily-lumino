...
MCP 的修改会直接保存并立即生效。公开范围仍由资料中的 `show_email`、`status_public`，以及链接和收藏卡片的 `is_public` 控制。创建链接和收藏卡片时默认公开，若希望隐藏，应明确传入 `is_public=false`。公开收藏可通过 `is_featured=true` 加入前厅横向收藏架，精选数量不受限制；隐藏卡片不能同时设为精选。
