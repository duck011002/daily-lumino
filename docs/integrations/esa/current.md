...
- 用户确认已在 ESA 侧配置按客户端 IP 统计的 `200 次/10 秒` 限流。该阈值与 ESA 官方智能限流“中等”初始档一致；当前未用生产突发流量触发规则，具体动作、自动调节和命中日志以控制台为准。
...
- 执行位置：ESA 边缘，先于源站 FastAPI/Nginx 业务处理，可减少异常流量回源。
- 当前配置：用户确认 `200 requests / 10 seconds / client IP`。
- 当前仓库：FastAPI 与 Nginx 没有第二层 `limit_req`、SlowAPI 或同类单 IP 页面限流。
- 已验证内容：正常低频访问、缓存与私有隔离未受影响。
- 未验证内容：没有对生产站点发送超过阈值的突发请求，因此不能把“规则已配置”写成“阻断动作已压测验证”。
