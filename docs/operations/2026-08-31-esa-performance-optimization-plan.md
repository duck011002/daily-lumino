...
## 9. 冻结决定与边缘限流补充（2026-09-01）

剩余 8 项已写入根目录 `TODO.md`，默认冻结。除非用户明确点名解冻某项，否则 Agent 不得继续实施。

用户确认 ESA 已配置按客户端 IP 的 200 次/10 秒边缘限流。仓库与生产 Nginx 当前没有重复的源站单 IP 限流。该设计把异常流量优先挡在边缘，但本轮没有用生产突发流量触发阈值；正常低频回归通过，阻断动作仍以控制台配置与命中日志为准。
