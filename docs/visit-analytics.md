...
后端也兼容 ESA 的标准 `ali-ip-country` 请求头，并使用随应用发布的 ip2region IPv4/IPv6 离线数据库补齐缺失字段。离线查询不会把访客 IP 发送给第三方；ESA 的可信地域头优先级更高。后台汇总流程会自动回填仍在 48 小时原始明细保留窗口内的 `XX` 记录；超过该窗口只剩匿名日汇总，已没有 IP 可用于补算。
