...
def blog_test_setup(client: TestClient, db):
    ...
    return {"normal": normal_cookies, "admin": admin_cookies}
...
def test_admin_blog_crud(client: TestClient, blog_test_setup, db):
    ...
    res_dup = client.post(
        "/api/admin/blog/posts",
        json={"title": "重复博客", "slug": "test-blog", "content": "内容"},
        cookies=admin_cookies,
    )
    ...
    patch_res = client.patch(
        ...
        json={"title": "修改后的博客", "slug": "updated-blog", "is_published": True},
        cookies=admin_cookies,
    )
...
def test_public_post_detail_read_is_cacheable_and_view_write_is_explicit(
    ...
    author = User(
        ...
        password=hash_password("password123"),
        display_name="Detail Cache Writer",
        is_active=True,
    )
    post = BlogPost(
        title="Cacheable article detail",
        slug="cacheable-article-detail",
        content="The read endpoint must not increment views.",
        author=author,
        is_public=True,
        is_published=True,
        view_count=5,
    ...
    first_read = client.get("/api/public/blog/posts/cacheable-article-detail")
    second_read = client.get("/api/public/blog/posts/cacheable-article-detail")

    assert first_read.status_code == 200
    assert second_read.status_code == 200
    assert first_read.json()["view_count"] == 5
    assert second_read.json()["view_count"] == 5
    assert first_read.headers["cache-control"] == (
        "public, max-age=0, s-maxage=60, stale-while-revalidate=120"
    )
    db.expire_all()
    assert db.scalar(select(BlogPost).where(BlogPost.slug == post.slug)).view_count == 5

    recorded = client.post("/api/blog/posts/cacheable-article-detail/view")

    assert recorded.status_code == 204
    db.expire_all()
    assert db.scalar(select(BlogPost).where(BlogPost.slug == post.slug)).view_count == 6
    assert client.post("/api/blog/posts/missing-article/view").status_code == 404
